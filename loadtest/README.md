...
This directory contains the self-contained Artillery suite for the manual **Load Test- Drop Simulation** GitHub Action. It is built for The List dev, stage, CA prod, MI prod, and NJ prod environments and uses AWS Fargate for generated load.
...
- `env`: `stage`, `dev`, `prod-ca`, `prod-mi`, or `prod-nj`.
...
`prod-ca` targets the live production site at `https://thelist.710labs.com`, `prod-mi` targets `https://thelist-mi.710labs.com`, and `prod-nj` targets `https://thelist-nj.710labs.com`. For production verification runs, prefer `mode=dry` or `place_orders=false` unless production order placement is intentional.
