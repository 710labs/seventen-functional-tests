...
test.describe('Admin Split Order', () => {
	...
	test(`User Can Split Order - Legacy Split @CA`, async ({ page, browserName }, workerInfo) => {
	...
	test.skip(`User Can Split Order - Auto Split @CA`, async ({ page, browserName }, workerInfo) => {
		const adminLoginPage = new AdminLogin(page)
		const editOrderPage = new EditOrderPage(page)
		await test.step('Login Admin', async () => {
			await adminLoginPage.login()
...
test.describe.skip('Auto Assign Fulfilment', () => {
	const zipCode = '90210'
	const orderQuanity = 6
	var orderTotals
	var orderNumber
	...
	test(`New Customer - No Previous Orders - Defaults to Pickup`, async ({
		page,
		browserName,
	}, workerInfo) => {
		const adminLoginPage = new AdminLogin(page)
	...
	test(`Existing Customer - Previou Order Pickup - Defaults to Pickup`, async ({
		page,
		browserName,
	}, workerInfo) => {
		const adminLoginPage = new AdminLogin(page)
	...
	test(`Existing Customer - Previou Order Delevery - Defaults to Delivery`, async ({
		page,
		browserName,
	}, workerInfo) => {
		const adminLoginPage = new AdminLogin(page)
