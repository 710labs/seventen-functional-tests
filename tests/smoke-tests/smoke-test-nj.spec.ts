...
test.describe('Basic Acceptance Tests NJ @smoke', () => {
	...
	test(`Basic Acceptance Test - Medical @medical @smoke`, async ({ page, browserName, context }, workerInfo) => {
		...
		test(`Basic Acceptance Test - Recreational @recreational @smoke`, async ({ page, browserName, context }, workerInfo) => {
		test.skip(workerInfo.project.name === 'Mobile Chrome')
		const apiContext = await request.newContext({
			baseURL: `${process.env.BASE_URL}${process.env.QA_ENDPOINT}`,
			extraHTTPHeaders: {
		...
		await test.step('Add Products to Cart', async () => {
			await shopPage.addProductsToCart(orderQuanity, mobile, 'Pickup', 'Recreational')
			cartTotals = await cartPage.verifyCart(zipCode)
		})
